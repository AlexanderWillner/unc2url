...
VERSION=$(shell $(PYTHON) -c 'import unc2url; print(unc2url.__version__)')
SHELL := /usr/bin/env bash -euo pipefail -c

help: ## Print help for each target
	$(info A UNC to URL Converter.)
	$(info =======================)
	$(info )
...
test: ## Test the code
	...
	@coverage erase
	@coverage run -a -m $(SRC_TEST).test_$(MAIN)
	@coverage report
	@coverage html
...
clean: ## Cleanup
	...
	@rm -rf $(SRC_TEST)/.mypy_cache .mypy_cache
	@rm -f .coverage
...
feedback: ## Give feedback
	@open https://github.com/alexanderwillner/unc2url/issues

release: build ## Create a new release
	@type gh >/dev/null 2>&1 || (echo "Run e.g. 'brew install gh' first." >&2 ; exit 1)
	@gh release create "v$(VERSION)" -t "Release $(VERSION) ($(DATE))" 'dist/$(MAIN)-$(VERSION).tar.gz'
